...
public class OldConfigTreeDeserializer implements JsonDeserializer<ConfigTree> {
    ...
    private static void deserializeCategory(JsonObject json) {
        ...
        for(JsonElement current : json.getAsJsonArray("children")) {
            ...
            if(childObject.has("value")) {
                OldConfigTreeDeserializer.deserializeOption(childObject, cat);
            }
            if(childObject.has("children")) {
                OldConfigTreeDeserializer.deserializeGroup(childObject, cat);
            }
        }
    }
    ...
    private static void deserializeGroup(JsonObject json, CategoryNode parent) {
        ...
        for (JsonElement current : json.getAsJsonArray("children")) {
            OldConfigTreeDeserializer.deserializeOption((JsonObject) current, group);
        }
    }
